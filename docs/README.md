...
public repository, production hosting, live canonical reads, and one finalized
OKX-signed root creation with canonical reload are now verified.
...
| State model | `PASS` | Entity-isolated grant trees, immutable parent links/clauses, bounded depth, nonce/idempotency, monotonic attempt numbers with the latest canonical review, technical retry, terminal ambiguity locks, denial, revocation, and fail-closed reads are explicit. |
...
| Claim-to-code | `PASS` | Every retained claim maps to an implemented method/view, local test, frontend control/test, and sanitized Studionet/browser evidence; one OKX root write has finalized production proof. |
...
| Scope honesty | `PASS` | Studionet behavior, production hosting, canonical reads, and one OKX root write are claimed only from recorded evidence; broader browser-write coverage, adoption, performance, security audit, and Portal outcome are not claimed. |
...
- Canonical active, denied, ambiguous, technical-retryable, revoked, and
  derived-expired outcomes.
...
| Grant detail `/grants/:grantId` | Understand lineage, effective authority, clauses, history, and legal next actions | Delegate, review/retry, revoke, or check access when eligible | Loading, not found, active, proposed, denied, ambiguous, technical retryable, revoked, expired, read unavailable | Core outcome before technical context; lineage becomes vertical list |
...
The contract stores these four top-level maps plus one append-only discovery
...
child: ABSENT -> PROPOSED -> ACTIVE | AMBIGUOUS | DENIED | RETRYABLE
technical retry: RETRYABLE -> ACTIVE | AMBIGUOUS | DENIED | RETRYABLE
semantic ambiguity: AMBIGUOUS -> terminal non-authorizing state
protective: PROPOSED | RETRYABLE | AMBIGUOUS | ACTIVE | DENIED -> REVOKED
...
- Global grant-ID uniqueness prevents duplicate roots/children and cross-tree
  overwrites.
- Final `ACTIVE`, `AMBIGUOUS`, `DENIED`, and `REVOKED` states reject duplicate
  semantic review or revocation. Only technical `RETRYABLE` review is an explicit
  new attempt and increments the stored counter exactly once after settlement.
- A Keccak-256 fingerprint binds parent ID/version, child grantee, canonical
  capability/resource CSV, expiry/depth constraints, and canonical child clauses.
  Fingerprints exclude child ID and nonce, so recreating unchanged ambiguous
  authority under another ID cannot bypass the terminal lock.
...
| `propose_child_grant` | Caller equals active parent grantee | Parent `ACTIVE` and effective; depth available | Missing/inactive/revoked/expired/depth-full parent; existing child or ambiguity-locked definition | `now < parent.expires_at`; `now < child.expires_at <= parent.expires_at`; equality late | Duplicate child, locked fingerprint, or `sender|propose_child_grant|nonce` rejects before mutation | Non-payable; 0 GEN; accounting unchanged | `get_grant`, `get_review`, `is_effective`, `can_invoke` | Wrong caller/state, wider scopes, clause coverage/kind, time/depth boundaries, replay, cross-ID ambiguity copy, child/index/nonce unchanged |
| `review_child_grant` | Caller equals recorded child grantor | Child `PROPOSED` or technical `RETRYABLE`; complete chain effective | Root/missing/active/ambiguous/denied/revoked/expired/inactive-ancestor child | `now` strictly before child and every ancestor expiry | Final states reject; each legal technical retry increments attempt once | Non-payable; 0 GEN; accounting unchanged | `get_grant`, `get_review`, `is_effective`, `can_invoke` | Wrong caller/state, expiry -1/equal/+1 with stale phase, malformed/malicious output, semantic ambiguity replay, disagreement, duplicate final review, unchanged authority |
...
an OKX-signed root write finalized and reloaded canonical authority in production.
...
- Invalid/unverifiable attestation: `N/A` external attestation; malformed or
  invariant-invalid semantic output is non-authorizing and technically
  retryable/unchanged, while semantic ambiguity is terminal for its fingerprint.
...
| --- | --- | --- | --- | --- | --- | --- |
| Root authority is authenticated and isolated | `create_root_grant`: absent -> `ACTIVE`; sender locked as root/grantor; append ID once | `get_grant`, `list_grant_ids`, `is_effective` | Wrong caller binding, duplicate ID/nonce, index isolation/pagination, time bounds | Root form, wallet gate, list/detail lifecycle/reload tests | Archived lifecycle under `docs/evidence/studionet/archive/0x20Cc…F396/`; active corpus root in `adversarial-evaluator.json`; browser evidence in `docs/evidence/browser/wallet-lifecycle.md` | Local + Studionet + OKX browser PASS |
| A child cannot exceed deterministic parent scope | `propose_child_grant`: absent -> `PROPOSED` after subset/depth/time/clause checks | `get_grant`, `is_effective` | Wider cap/resource/time/depth, clause mismatch, wrong actor/state | Delegate form parent gate/validation/finality tests | Archived full lifecycle plus active-address adversarial proposals in `docs/evidence/studionet/adversarial-evaluator.json` | Local + Studionet PASS; browser wrapper/tests PASS |
| Only validator-agreed qualitative attenuation activates a child | `review_child_grant`: `PROPOSED/technical RETRYABLE -> ACTIVE/DENIED/AMBIGUOUS/RETRYABLE`; semantic clause-pair locks reject unchanged ambiguity | `get_review`, `get_grant`, `is_effective` | Same-child/cross-ID replay, unrelated expiry/grantee/scope/order/whitespace changes, ambiguous-clause revision, technical retry, malicious outputs, total coverage/invariants | Named ambiguous clauses, terminal-vs-technical guidance, truthful signature/receipt stages, canonical reload | `docs/evidence/studionet/ambiguity-lock-remediation.md` and `adversarial-evaluator.json` prove exact deployed-source provenance and three finalized live injection reviews | Local + active Studionet PASS; 3/3 adversarial cases non-authorizing |
| Revocation/expiry fail-closes every descendant without partial cascade | `revoke_grant`: target -> `REVOKED`; descendant result derived | `is_effective`, `can_invoke`, `get_grant` | Deep chain, expired revoke, wrong actor, unrelated tree, no double revoke | Eligible revoke control and descendant refresh | Archived prior-address lifecycle (`REVOKE_ROOT`) plus current source-provenance proof | Local + Studionet PASS; browser wrapper/tests PASS |
| Exact protected actions bind the authenticated actor and fail closed | No write; actor equality plus live bounded chain/scope evaluation | `can_invoke` exact reason | Actor mismatch, all reason enums, invalid time, boundary equality, missing grant, stale phase | `/checks` connected-actor, mismatch, allowed/denied/unavailable tests | Archived prior-address `phase-8-verification.md`; active contract exact-source proof in `ambiguity-lock-remediation.md` | Remediated source/local + Studionet PASS; production browser proof pending |
...
- `docs/evidence/studionet/ambiguity-lock-remediation.md`
- `docs/evidence/studionet/adversarial-evaluator.json`
- historical full lifecycle under `docs/evidence/studionet/archive/`
...
- Contract implementation, lint/direct tests, semantic mocks, deployment,
  ...
  transport and one OKX-signed root transaction are verified.
